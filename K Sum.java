...
第 道题3sum，返回有没有就 。但他 按套 出牌，问我最直接的解法是 么。我楞  下，说三个for循环? 叔居 然很开 地说就是这样。。。然后贴   解法的代码让我分析，让我回答这个代码有 么问题。具体代码就是三个for循环，但是index 都是从0开始，所以会有重复 同 个数的问题。然后问我怎么fix这个bug, 就是把每个循环的起始index变成i+1，i是上 层循环的当前 index。接下来就问我时间复杂度，有没有 优解。
